...
The `fetch_firms()` function retrieves fire detections using the FIRMS API. You must provide a region (AOI), time window (`start_date`, `end_date`), and optionally choose the dataset (`VIIRS_SNPP_NRT` or `MODIS_NRT`). You can also filter fires by confidence category (`l`, `n`, `h`).

```r
firms_raw <- fetch_firms(
  api_key = api_key,
  region_sf = region_sf,
